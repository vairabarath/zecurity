...
## Steps
- [x] Badge states: Healthy (≤24h) green, Delayed (≤72h) amber, Disconnected (>72h or null) red, Disabled grey.
- [x] Render `lastSyncAt` ("last synced 3h ago" / "never") next to the badge.
- [x] Only show the badge for SCIM-capable connections — gate on **`scimEnabled`** (present on `WorkspaceIdpConnection`), plus `lastSyncAt` being non-null for the "has ever synced" case. Do **not** use `scimEnabledAllowed`: it exists only on `IdpTestResult`, not on the connection type (see FE-1 Audit notes).
...
- **2026-08-26 — implemented (unverified).** New `admin/src/components/scim/IdentityHealthBadge.tsx` (pure presentational, maps the four backend strings to `StatusPill` tones ok/warn/danger/muted, renders `lastSyncAt` via existing `relativeTime`, returns null when `!scimEnabled` — no client-side threshold math). Mounted in `IdpConnectionDetail.tsx` header (extended the local `IdpConnectionDetailData` type with `identityHealth`/`lastSyncAt`/`scimEnabled`) and in `IdpConnections.tsx` per-row (replaced the inline `synced {relativeTime}` span). No GraphQL/codegen change — `GetIdpConnections` already selected the fields. Gates: `codegen` no-op, `build` green, `test` 16/16, `lint` delta = 0 (18 pre-existing problems, byte-identical to stashed baseline; new file not flagged). **Manual half NOT run:** visual check of all four states against seeded `lastSyncAt` values requires a live IdP/data and cannot be exercised here — hence `implemented-unverified`, not `done`.
