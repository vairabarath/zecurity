...
## Residual — relay certificate lifetime

Independently re-verified against the code on 2026-09-05 (second audit, reaching the same
conclusion as the 2026-08-10 acceptance above). Two details worth recording that the decision
block does not name:

- **A revoked relay is blocked by the database as well as by the CRL.** `RecordHeartbeat` and
  `MarkProvisioned` (`controller/internal/relay/store.go`) both gate on
  `status NOT IN ('revoked','deleted')`, so a revoked relay can neither heartbeat nor
  re-provision even if a verifier's CRL were stale. Defence in depth on the authority side.
- **Controller-side revocation is near-immediate, not 60s.** Beyond the periodic refresh, the
  `OnRelayRevoked` hook (`cmd/server/main.go`) refreshes `RelayRevocationChecker` on demand at
  the moment of revocation.

**Residual, non-blocking:** the relay cert TTL is still 30 days (`RELAY_CERT_TTL`) and there is
no relay renewal path — `internal/relay/store.go` still says "and future renewal". Option C's
recommendation to shorten it was explicitly defence-in-depth, and its premise ("the 30-day relay
cert is the weak link") assumed no revocation enforcement existed. With fail-closed CRL checks at
every hop plus the DB status gate, this is a hardening item, not a hole. Track it with the relay
lifecycle work.
