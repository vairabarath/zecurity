---
type: decision
status: accepted — implemented and verified live 2026-08-28 (Gate 3 closed: 7 pass, 1 design-incompatible)
date: 2026-08-27
amended: 2026-08-27
related:
...
through the OS with no `hosts` entry; `down` and `SIGKILL` both leave the user's DNS intact. TLS/SNI now passes with a real certificate and a negative control, and split-tunnel
consistency holds by construction (this client has no full-tunnel mode). One item,
