...
- [x] **M18-10** Transaction **rollback** removes the outbox event.
- [x] **M18-10** Transaction **commit** persists the outbox event.
- [x] **M18-10** **Concurrent claiming** does not duplicate — two+ workers never receive the same
      eligible event in the same lease period.
- [x] **M18-10** Unique `lease_id` + `claimed_at` assigned per claim.
- [x] **M18-10** **Stale `MarkDone`** after lease replacement affects zero rows.
- [x] **M18-10** **Stale `MarkFailed`** after lease replacement affects zero rows.
- [x] **M18-10** **Abandoned-event recovery** (reaper resets expired `processing` → retryable
      `failed`, +1 retry, backoff, cleared lease).
- [x] **M18-10** **Lease-replacement race** — reaper cannot reclaim a row after its expired lease
      was already replaced by a newer lease (the lease-A-expires / lease-B-claimed case).
- [x] **M18-10** **Retry exhaustion** — repeated crash/reap cycles reach `max_retries`; event
      stays `failed`, observable, permanently ineligible.
- [x] **M18-10** **Unknown-handler terminal behavior** — `failed`, `retry_count=max_retries`,
      `next_attempt_at=NULL`, `last_error` set; not reclaimed forever.
- [x] **M18-10** **Workspace-deletion protection** — deleting a workspace cannot cascade-delete
      committed events (`ON DELETE RESTRICT` blocks it).
- [x] **M18-10** **Terminal-recovery authorization** — `Recover` is terminal-only, requires a
      reason, requires operator identity, preserves retry budget unless `resetRetryBudget`.

## Steps — unit tests

- [x] **M18-10** Retry/backoff: `retry_count`, backoff values, and `next_attempt_at` with a
      deterministic injected clock and jitter.
- [x] **M18-10** `last_error` truncates at **4096 bytes**.
- [x] **M18-10** Graceful shutdown (context cancellation) — claim loop + reaper exit, no goroutine
      leaks.
- [x] **M18-10** Processor behavior: success → `done`; handler failure → retry scheduled; retry
      exhaustion → terminal `failed`; unknown event → terminal; claim → dispatch → complete cycle.
- [x] **M18-10** `correlation_id` preserved through processing and logging.
- [x] **M18-10** Idempotency: repeated delivery is safe (handler-level test — handler contract is
