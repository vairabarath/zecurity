...
`ConnectorDetail` page renders the connector record + `installCommand` in a copyable code block. Connector still has `status='pending'` so the page shows a "waiting for first check-in" banner.
