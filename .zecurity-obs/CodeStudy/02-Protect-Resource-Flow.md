...
🔴 **Finding 5 (important) — deleting a `protected` resource orphans the shield firewall rule + guard contradicts its error.**
`SoftDelete` (`:318-334`) guards only `status != 'protecting'`, so it **allows deleting a `protected` row** — but the error says *"must be unprotected before deleting"*. It's a hard `DELETE`, so no `remove` is sent to the shield; the shield rebuilds its chain from its own active set and keeps enforcing the orphaned nftables rule. Later shield re-verify acks hit `RecordAck`'s `WHERE … deleted_at IS NULL` → 0 rows, silent no-op. No console path to clean it up. **Fix (needs policy decision):** restrict delete to `status IN ('pending','failed','unprotected')` to match the message, OR enqueue a `remove` as part of delete.

🟠 **Finding 6 — no controller-side backstop out of `protecting` (= Piece 2 Finding 4, confirmed at DB layer).**
A row leaves `protecting` ONLY via `RecordAck` or reconnect replay (`GetPendingForShield`). No timeout sweep (`protecting` too long → `failed`), no periodic re-push. With Piece 2 Finding 4 (discarded online send error) + no reconnect → **`protecting` forever**, UI polls indefinitely. Delivery mechanics = Piece 4; state layer has no safety net. **Deferred to Piece 4.**
...
6. **Delete of `protected` orphans shield firewall rule** (Piece 3, Finding 5) — `SoftDelete` allows deleting `protected` (guard contradicts its error msg); hard-delete sends no `remove`, shield keeps enforcing. **Open — needs delete-policy decision.**
7. **Protect/unprotect shield-active asymmetry** (Piece 3, Finding 7) — unprotect against a dead shield can stick in `protecting` forever. **Open — decision deferred.**
8. **Vestigial soft-delete + stale index** (Piece 3, Findings 8/9) — `deleted_at`/`'deleted'` scaffolding is dead (hard-delete); `idx_resources_managing` matches zero rows post-rename. **Open — low-risk cleanup.**
