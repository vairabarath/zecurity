...
| ✅ [PENDING-16](PENDING-16-Resource-Policy-Device-Profile-Binding.md) | Resource Policy → Device Profile Binding — **done, Sprint 19 Track A on branch `pending-16`** (not yet merged to `fixed-pendings`). `Resource → Resource Policy → Device Profile(s)` is the authorization path; `device_profiles.mode` reaches no authorization decision. Phases 1–10 complete: cutover in P5, propagation in P6, admin UI in P7, real Linux hardware end-to-end in P8, full test execution in P9, final verification in P10. | policy | P1 | PENDING-08, PENDING-09 |
