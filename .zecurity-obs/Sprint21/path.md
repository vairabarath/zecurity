...
# Sprint 21 — Provider Dashboard Phase 2: Secure Read-Only Console
...
Give the provider operator a **read-only console that shows true state**, behind **hardened provider authentication**.

Sprint 20 made controller state true. Sprint 21 exposes it, and first hardens the provider identity boundary, because the new read APIs let a super-admin read across every tenant (D-15).
...
No provider UI                                         Separate read-only React console (D-18)
...
| **D-07…D-13, D-17** | Suspension, deletion, audit immutability roles and destructive-action safeguards are **not in this sprint**. The console is **read-only**. Don't change the `workspaces.status='active'` predicates. |
...
| Console token | Held in memory, plus `sessionStorage` so a reload keeps it. 15 min provider TTL; **no refresh token**, so the user logs in again on expiry. | D-16 doesn't add refresh; the console is read-only. |
...
|--------|--------|------|------|
| **M1** | Sathiya | Go + React | Sprint 20 live verification; provider read actions and read APIs; provider console |
| **M2** | Barath | Go + Rust | Provider identity hardening (D-16), incl. the console login contract; KI-1, KI-2, KI-3; `.env.example` JWT cleanup; DEV-1 docs |

**Shared review** (both members approve): `internal/middleware/provider.go`, `decide()` read actions in `internal/provider/authz.go`, and the auth boundary tests (`AT-H` and `AT-R.1`).

## Critical Rule: Conflict Zones

| File | Who | Rule |
|------|-----|------|
| `controller/cmd/server/main.go` | M2 (H: provider config, logout route, CORS, callback) + M1 (R: read routes) | **M2-H lands first.** M1-R wires its routes after rebasing onto H. Surgical edits only. |
| `controller/internal/middleware/provider.go` | M2 (H) | M2 owns; shared review. M1 only consumes `RequireProvider`. |
| `controller/internal/provider/session.go`, `store.go` | M2 (H) | M2 owns. M1 adds new files (handlers) but doesn't edit these two. |
...
| `provider-console/` (new) | M1 (P) | M1 only. `admin/` is **not** modified. |
...
M2-H  Provider identity hardening (D-16)  (Day 1)  ── schema 037, key/issuer, sub/hd, generation, logout,
  ...
  ├──────────────► M1-R  Provider read actions + read APIs
  │                  (R1–R2 query services start Day 1; route wiring R3+ waits for H merged)
  │                    │
  │                    ▼
  └──────────────► M1-P  Provider console (needs H6 login contract + R endpoints)

M2-K  Sprint 20 cleanup: KI-1, KI-2, KI-3, JWT scrub   (after H — shares .env.example)
...
> **Day-1 parallelism:** M2-H, M1-V, and M1-R's query-service layer (R1–R2, no routing) all start immediately.
...
> See [[Sprint21/Member1-Go/Phase2-Provider-Read-APIs]].
...
- [ ] **M1-R3** Handlers + routes (after M2-H merged): `GET /provider/relays`, `/provider/relays/{id}`, `/provider/tenants`, `/provider/tenants/{id}` (needs OQ-1/OQ-2), `/provider/audit`, `/provider/certificates`.
- [ ] **M1-R4** Tests: role matrix per endpoint, data correctness, pagination, no secret fields; build gate.

### Phase P — M1: Provider Console (D-18)

> See [[Sprint21/Member1-Go/Phase3-Provider-Console]].

- [ ] **M1-P1** `provider-console/` scaffold (Vite/React 19/TS/Tailwind 4); UI primitives copied from `admin/`; dev proxy `/provider` → `:8080`.
- [ ] **M1-P2** Auth: login → Google → `/auth/callback#token=`, token store, logout, 401 → re-login.
- [ ] **M1-P3** Read-only pages: Relays (list + detail), Tenants (list + detail), Audit, Certificates.
- [ ] **M1-P4** Vitest tests; `npm run build` + `npm run lint`; deployment notes (own origin, network lock).

### Phase K — M2: Sprint 20 Cleanup

> See [[Sprint21/Member2-Go-Rust/Phase2-Sprint20-Cleanup]].
...
- [ ] The **read-only provider console** shows relays, tenants, audit and certificate health; it has **no mutating controls**; and it runs on its own origin.
...
- Provider **mutations** in the console (relay create, revoke or delete stay API-only). D-17 safeguards come with mutations.
...
4. The console is read-only. No button, form or route may call a mutating endpoint.
