...
- [x] **M2-D1** `proto/client/v1/client.proto` — Add three fields to `ACLSnapshot`:
  ```proto
...
> **Goal:** Controller populates per-connector relay fields in ACL snapshot from DB (ADR-014 Gap 1).
> **Depends on:** Day 1 proto done.
> See `Sprint10/Member2-Go/Phase1-ACL-Relay-Fields.md`.

- [x] **M2-A1** `controller/internal/policy/compiler.go` — Extend `CompileACLSnapshot` to populate `relay_addr`, `relay_spiffe_id` per connector:
  - LEFT JOIN `connector_relay_placement` and `relays` in `GetConnectorsForRemoteNetworks`
  - Populate `relay_addr` and `relay_spiffe_id` on each `ACLConnector` from DB placement row
  - Global workspace-level relay block retained for backward compat (old clients)
- [x] **M2-A2** `store.go` — Extend `RemoteNetworkConnectorsRow` with `RelayAddr` and `RelayID`; LEFT JOIN `connector_relay_placement` + `relays` in SQL (relay routing is DB-driven per ADR-014 — env var approach superseded)
- [x] **M2-A3** Build check: `cd controller && go build ./...` passes
...
- [x] **M2-B1** Write `Sprint10/Member2-Go/Phase2-SPIFFE-Relay-Spec.md` specifying:
...
- [x] **M2-B2** Review M3's `relay/src/spiffe.rs` once written — confirm exact-match logic matches the spec
...
- [x] **M3-C1** `relay/Cargo.toml` — New workspace member. Deps: `quinn`, `rustls`, `tokio`, `serde_json`, `tracing`, `anyhow`, `bytes`
- [x] **M3-C2** `relay/src/main.rs` — QUIC listener on `:9093`, ALPN `ztna-relay-v1`, loads TLS cert from env `RELAY_TLS_CERT` / `RELAY_TLS_KEY`, calls `relay::start()`
- [x] **M3-C3** `relay/src/listener.rs` — `start()`: creates QUIC endpoint, accept loop, spawns `handle_connection()` per peer
- [x] **M3-C4** `relay/src/state.rs` — `RelayState`:
  ```rust
...
- [x] **M3-C5** `relay/src/session.rs` — `handle_connection()`:
...
- [x] **M3-C6** `relay/src/spiffe.rs` — `validate_spiffe(cert, expected_prefix)` — extracts URI SAN from peer cert, validates exact connector/client format; workspace isolation via trust domain comparison
- [x] **M3-C7** Build check: `cd relay && cargo build` passes
...
- [x] **M3-E1** `client/src/relay_pool.rs` — NEW:
...
- [x] **M3-E2** `client/src/tunnel_pool.rs` — MODIFY: In `open_stream()` (or equivalent), wrap direct QUIC connect in a 2s timeout:
  ```rust
  ...
  ```
- [x] **M3-E3** `client/src/daemon.rs` — Pass `relay_addr` and `connector_id` from ACL snapshot through to tunnel pool when starting smoltcp loop
- [x] **M3-E4** Build check: `cd client && cargo build` passes

---

## Final Build Gates (all must pass)

- [x] `buf generate` (repo root) — clean
