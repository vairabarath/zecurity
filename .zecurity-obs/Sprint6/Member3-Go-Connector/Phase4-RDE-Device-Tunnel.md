...
sprint: 6
member: M3
phase: Phase4-RDE-Device-Tunnel
depends_on:
  - M2-D1-A (shield.proto TunnelOpen/Opened/Data/Close)
  - buf generate
  - M3-D3 (connector discovery modules)
tags:
  - rust
  - connector
  - rde
...
# M3 Phase 4 — RDE: Remote Device Extension (Device Tunnel)
...
struct TunnelResponse {
    ...
    error:     Option<String>,
    /// QUIC address for client to upgrade to (Option C discovery).
...
/// Start the QUIC/UDP device tunnel listener on the same port as TLS/TCP but over UDP.
/// ALPN: `ztna-tunnel-v1`
/// On success, advertises the external address via `device_tunnel::set_quic_advertise_addr()`
/// so TLS responses include `quic_addr` for client discovery.
pub async fn listen(
    addr: &str,
    advertise_addr: &str,
    controller_http_url: String,
    ...
    // Advertise the externally reachable address (may differ from bind addr behind NAT).
...
The `AgentTunnelHub` already exists. Modify `handle_incoming()` to also handle tunnel messages coming back from the **Shield** via the Control stream:

The Shield sends `ShieldControlMessage` variants: `TunnelOpened`, `TunnelData`, `TunnelClose`. The connector's agent_server receives these and must forward them into the hub's session dispatch.

In `agent_server.rs`, add handling for when the Shield Control stream sends these messages:
...
Add these `dispatch_*` methods to `AgentTunnelHub` (or use the existing `handle_incoming` pattern, mapping proto variants to `TunnelEvent`).

When the connector wants to **open** a tunnel to the shield (from `open_relay_session`), send via the Shield Control stream:

```rust
// Send TunnelOpen to shield via the control stream sender
...
### 4. `connector/src/main.rs` (MODIFY)

Start both device tunnel listeners:
...
tokio::spawn(quic_listener::listen(
    "0.0.0.0:9092",
    &format!("{}:9092", lan_ip),  // advertise_addr
    controller_http_url.clone(),
    cert_store.clone(),
    acl.clone(),
    tunnel_hub.clone(),
...
Add `mod device_tunnel;` and `mod quic_listener;`.

---

### 5. Controller: `/api/device/check-access` endpoint (Go)

Add an HTTP endpoint to the controller that the connector calls as a fallback when the local policy cache misses:
...
// Handler validates a device JWT and checks if the requesting device
// may access the given destination:port/protocol.
// Called by connector as a policy cache fallback (not hot path).
func CheckAccessHandler(db *sql.DB) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        ...
        // 4. Check device ACL (Sprint 7) — for now allow if resource exists
        // 5. Respond with { allowed: true/false, resource_id: "..." }
        var req CheckAccessRequest
        if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
            http.Error(w, "bad request", http.StatusBadRequest)
            return
        }
        // TODO Sprint 7: real ACL check. For now: allowed = resource exists.
...
> **Sprint 7 note:** Real ACL policy enforcement goes here. For Sprint 6, the endpoint exists so the connector can call it — it returns `allowed=true` if a resource matching the destination exists in the workspace.
