...
├── connector/           Rust agent (enrollment + heartbeat + auto-update)
...
### Connector Subsystem
- `internal/connector/config.go` — ConnectorConfig struct (CertTTL, HeartbeatInterval, etc.)
- `internal/connector/token.go` — JWT enrollment token generation + Redis JTI burn
- `internal/connector/enrollment.go` — gRPC Enroll handler (verify JWT, sign CSR, return cert)
- `internal/connector/heartbeat.go` — gRPC Heartbeat handler + disconnect watcher goroutine
...
- `src/heartbeat.rs` — mTLS heartbeat loop (SPIFFE preflight, tonic channel, 30s interval, exponential backoff)
- `src/crypto.rs` — EC P-384 keygen, PEM I/O, CSR building
...
### Proto
- `proto/connector.proto` — ConnectorService (Enroll + Heartbeat RPCs)
...
- `Cross.toml` — cross-compilation config (protoc inside Docker)
- `Dockerfile` — multi-stage build for containerized deployment
- `docker-compose.yml` — example Docker deployment
...
- `Cross.toml` installs protoc inside the Docker container
...
- **7-day cert validity**: Short-lived certs, auto-renewal planned for next sprint
