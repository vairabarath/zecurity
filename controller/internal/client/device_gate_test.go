...
// to own devices.
func deviceGateTestHarness(t *testing.T) (pool *pgxpool.Pool, ctx context.Context, wsID, userID string) {
	t.Helper()
	...
	return pool, ctx, wsID, userID
}

// TestDeviceGateDirectiveDerivation exercises every {status, revoked_at}
// combination from Track2-Device-Trust-Directive.md D-C: re_enroll_required
// takes priority over revoked (derived from revoked_at, never duplicated
// into status) takes priority over renew_pending, else none.
func TestDeviceGateDirectiveDerivation(t *testing.T) {
	pool, ctx, wsID, userID := deviceGateTestHarness(t)
	...
	cases := []struct {
		...
		{"active", "active", false, clientv1.DeviceDirective_DIRECTIVE_NONE, false},
		{"revoked", "active", true, clientv1.DeviceDirective_DIRECTIVE_REVOKED, true},
		{"re_enroll_required beats revoked", "re_enroll_required", true, clientv1.DeviceDirective_DIRECTIVE_RE_ENROLL_REQUIRED, true},
		{"re_enroll_required alone", "re_enroll_required", false, clientv1.DeviceDirective_DIRECTIVE_RE_ENROLL_REQUIRED, true},
		{"renew_pending", "renew_pending", false, clientv1.DeviceDirective_DIRECTIVE_RENEW_SOON, true},
	}
