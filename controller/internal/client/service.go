...
func (s *Service) InitiateAuth(ctx context.Context, req *clientv1.InitiateAuthRequest) (*clientv1.InitiateAuthResponse, error) {
	...
	if _, err := rand.Read(rawVerifier); err != nil { //fills 0's and 1's randome order
		return nil, status.Errorf(codes.Internal, "generate google pkce verifier: %v", err)
	}
	googleVerifier := base64.RawURLEncoding.EncodeToString(rawVerifier) // create code verifier 
	googleChallenge := sha256b64url([]byte(googleVerifier)) // create code challenge for the google 

	sessionID, err := newSessionID() // newSessionID() -> creates 24 randome 0's and 1's bytes then convert to the hex values
	if err != nil {
	...
	putSession(sessionID, &authSession{
		...
		LocalRedirectURI:   req.GetLocalRedirectUri(), // stored in the session and send to the code generated by the controller is send to the client
...
func (s *Service) AuthCallbackHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		claims, err := auth.VerifyGoogleIDToken(r.Context(), tokens.IDToken, s.clientGoogleClientID) // checks is this is provided by the google and client id and the email is verified by the google and contains email sub is the constant
		...
		ctrlCode, err := newCtrlCode() // new 32 bit random string from the bytes
		if err != nil {
			http.Error(w, "internal error", http.StatusInternalServerError)
			return
		}

		if !updateSessionCtrlCode(sessionID, claims.Email, claims.Sub, ctrlCode, time.Now().Add(60*time.Second)) {// stores session in and email in the 
