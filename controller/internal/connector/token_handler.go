...
func RegenerateTokenHandler(pool *pgxpool.Pool, cfg Config, rdb *redis.Client) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Extract connector ID from URL: /connectors/{id}/token
		// URL path is /connectors/<uuid>/token
		parts := strings.Split(strings.Trim(r.URL.Path, "/"), "/")
		if len(parts) != 3 || parts[0] != "connectors" || parts[2] != "token" {
		...
		// Verify connector is pending and belongs to this tenant
		var status string
		...
		// Load intermediate CA fingerprint
		var certPEM string
		err = pool.QueryRow(ctx,
		...
		// Store new JTI in Redis (invalidates old one)
		...
		// Build install command (same format as GenerateConnectorToken mutation)
