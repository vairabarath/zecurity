#!/usr/bin/env bash
# shield-local-install.sh — ZECURITY shield local installer
#
# Installs:
#   - zecurity system user
#   - /usr/local/bin/zecurity-shield                 (from local binary)
#   - /etc/zecurity/shield.conf                      (config)
#   - /var/lib/zecurity-shield/                      (state dir)
...
Installs the ZECURITY shield locally.

Required env vars:
  CONTROLLER_ADDR       gRPC host:port for enrollment (e.g. localhost:9090)
...
if ! command -v nft >/dev/null 2>&1; then
    err "nftables not found. Please install it (e.g., apt install nftables)"
fi
...
UNITS_SRC="${REPO_ROOT}/shield/systemd"
...
cat > "$CONFIG_FILE" <<EOF
# ZECURITY shield configuration — written by shield-local-install.sh
...
systemctl status zecurity-shield.service --no-pager --lines=5 || true
