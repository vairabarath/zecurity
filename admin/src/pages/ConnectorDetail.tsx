...
export default function ConnectorDetail() {
  ...
  // ── Find connector + its network from the flat networks+connectors query ──
  ...
  // ── Install command state (for pending connectors) ────────────────────────
  ...
  const INSTALL_COMMAND = useMemo(() => installCommand, [installCommand])
  ...
  const fetchInstallCommand = async () => {
    ...
    setTokenError(null)
    try {
      const resp = await fetch(`/connectors/${connectorId}/token`, {
        method: 'POST',
        credentials: 'include',
  ...
  // Auto-fetch token when connector is known to be pending
  useEffect(() => {
  ...
  const installCard = (
    <Card className="mt-8 mx-auto max-w-2xl text-left">
      <CardHeader>
        ...
        <CardDescription>
          Verify the addresses below, then copy and run the command on your server.
        </CardDescription>
      </CardHeader>
  ...
  // ── Installed (Active / Disconnected / Revoked) ───────────────────────────
  return (
    <div className="space-y-6">
      {/* Breadcrumb */}
      <motion.div
      ...
      >
        <Link to="/remote-networks" className="hover:text-foreground transition-colors">
          Remote Networks
        </Link>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          {isRevoked && (
            <Button
              variant="outline"
              className="gap-2 text-green-500 border-green-500/40 hover:text-green-600 hover:border-green-600"
              disabled
            >
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            <InfoRow
              ...
              value={
                <span className="font-mono text-muted-foreground text-xs">
                  {connector!.certNotAfter
                    ? new Date(connector!.certNotAfter).toLocaleString()
                    : '—'}
                </span>
              }
            />
