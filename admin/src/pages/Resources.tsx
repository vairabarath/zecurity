...
export default function Resources() {
  ...
  // Lazy initializers read location.state at mount — avoids calling setState
  // inside an effect body (which causes cascading renders).
  const [showAdd, setShowAdd] = useState<boolean>(() => {
    const s = location.state as ResourcePrefillState | null;
    return !!s?.createResourceDefaults;
  });
  const [prefill, setPrefill] = useState<
    ResourcePrefillState["createResourceDefaults"] | null
  >(() => {
    const s = location.state as ResourcePrefillState | null;
    return s?.createResourceDefaults ?? null;
  });
  ...
  useEffect(() => {
    const hasTransition = resources.some((resource) =>
      transitionalStates.has(resource.status),
    );
    startPolling(hasTransition ? 3000 : 30000);
  }, [resources, startPolling]);

  // Clear router nav state after consuming it — no setState calls, so no cascade.
  // Safe to include full deps: once navigate clears location.state the condition
  // is false on every subsequent run, making this effect a natural one-shot.
  useEffect(() => {
    if ((location.state as ResourcePrefillState | null)?.createResourceDefaults) {
  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Managed services protected and relayed through shields.
          </p>
        </div>
          ...
          {loading && !data ? (
            ...
            <div className="min-w-300">
              {resources.map((resource) => {
                ...
                const shieldOffline =
                  resource.shield?.status === ShieldStatus.Disconnected;
      ...
      <CreateResourceModal
        key={showAdd ? (prefill ? JSON.stringify(prefill) : 'open') : 'closed'}
        open={showAdd}
        onOpenChange={(open) => {
          setShowAdd(open);
