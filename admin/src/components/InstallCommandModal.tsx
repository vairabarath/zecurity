...
export function InstallCommandModal({ remoteNetworkId: fixedNetworkId, open, onClose }: InstallCommandModalProps) {
  ...
  // Only fetch networks when no fixed network is provided
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        {!installCommand && (
          <div className="grid gap-4 py-2">
            ...
            {/* Network selector — only shown when no fixed network */}
            {!fixedNetworkId && (
              <div className="grid gap-2">
                ...
                <select
                  ...
                  className="flex h-10 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm ring-offset-background focus:outline-none focus:ring-2 focus:ring-primary/30 disabled:cursor-not-allowed disabled:opacity-50"
                >
