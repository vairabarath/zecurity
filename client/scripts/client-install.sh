...
systemctl enable zecurity-client.service
...
log ""
log "OPTIONAL — automatic DNS for name-addressed resources:"
log "  managed hostnames resolve via a hosts entry by default (see"
log "  'zecurity-client resources'). To have the OS resolve them automatically,"
log "  install the privileged DNS helper — it is a SEPARATE, OPT-IN step because"
log "  it adds a root systemd service:"
log "    sudo dns-helper/scripts/dns-helper-install.sh"
log "  The client works fine without it; see ADR-023."
